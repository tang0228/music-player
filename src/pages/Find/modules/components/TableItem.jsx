...
import style from "./tableItem.module.less";
...
export default function TableItem(props) {
  ...
  return (
    <li className={utils.isEven(index) ? style["table-item"] +  ' ' + style['even'] : style["table-item"]}>
      ...
      <div className="name bd ellipsis-1">
        {item.ar.map((e, i) => (
          <Link to={'/artist?id=' + e.id } key={e.id}>{i === 0 ? e.name : `/${e.name}`}</Link>
        ))}
      </div>
